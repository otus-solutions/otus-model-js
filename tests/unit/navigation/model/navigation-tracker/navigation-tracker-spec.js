xdescribe('NavigationTracker', function() {
  var UNIT_NAME_FACTORY = 'otusjs.model.navigation.NavigationTrackerFactory';
  var Mock = {};
  var Inject = {};
  var tracker = {};
  ...
  describe('getItems method', function() {
    ...
    it('should return an object with all navigation tracking items as your properties ─ objectType', function() {
      var itemsContainer = tracker.getItems();
      var item = null;

      for (item in itemsContainer) {
        expect(itemsContainer[item].getObjectType()).toEqual('NavigationTrackingItem');
      }
    });

    it('should return an object with all navigation tracking items as your properties ─ count', function() {
      expect(Object.keys(tracker.getItems()).length).toEqual(6);
    });

    it('should return an object with all navigation tracking items as your properties ─ IDs and order', function() {
      var itemsContainer = tracker.getItems();
      var item = null;
      var position = 0;

      for (item in itemsContainer) {
        expect(itemsContainer[item].getID()).toEqual(Mock.itemsToTrack[position].templateID);
        ++position;
      }
    });
  ...
  describe('getCurrentItem method', function() {
    ...
    it('should return an item that corresponds to current visited item', function() {
    ...
    it('should return a NavigationTrackingItem', function() {
      tracker.visitItem('TPL1');
      expect(tracker.getCurrentItem().getObjectType()).toEqual('NavigationTrackingItem');
    });
  ...
  describe('getCurrentIndex method', function() {
    ...
    it('should return the index that corresponds to current item', function() {
      tracker.visitItem('TPL1');
  ...
  describe('visitItem method', function() {
    ...
    describe('when the item id is passed as parameter', function() {
      ...
      it('the current item id should be equal to the paramenter', function() {
        expect(tracker.getCurrentItem().getID()).toEqual('TPL4');
      });

      it('the items between the current item and the item to visit should have the state equal to SKIPPED', function() {
        expect(tracker.getItems().TPL1.isSkipped()).not.toEqual('VISITED');
        expect(tracker.getItems().TPL2.isSkipped()).not.toEqual('SKIPPED');
        expect(tracker.getItems().TPL3.isSkipped()).not.toEqual('SKIPPED');
        expect(tracker.getItems().TPL4.isSkipped()).not.toEqual('VISITED');
      });

      describe('and the item never was previously visited', function() {

        it('the current item state should be equal to VISITED', function() {
          expect(tracker.getCurrentItem().getState()).toEqual('VISITED');
        });

      });

      describe('and the item already was previously answered', function() {
        ...
        it('the current item state should be equal to ANSWERED', function() {
          expect(tracker.getCurrentItem().getState()).toEqual('ANSWERED');
        });

      });

      describe('and the item already was previously ignored', function() {
      ...
      describe('and the item already was previously skipped', function() {
  ...
  describe('updateCurrentItem method', function() {
    ...
    describe('when exists a current item', function() {
      ...
      describe('and exists an answer to the current item', function() {
        ...
        it('the current item state should be equal to ANSWERED', function() {
          tracker.updateCurrentItem(Mock.answeredItem);
          expect(tracker.getCurrentItem().isAnswered()).toBe(true);
        });
      ...
      describe('and the item is ignored', function() {
        ...
        it('the current item state should be equal to IGNORED', function() {
          tracker.updateCurrentItem(Mock.ignoredItem);
          expect(tracker.getCurrentItem().isIgnored()).toBe(true);
        });
  ...
  xdescribe('toJson method', function() {
    ...
    it('should return the instance parsed to JSON format ─ untouched json', function() {
      expect(Mock.untouchedJson).toEqual(tracker.toJson());
    });

    it('should return the instance parsed to JSON format ─ visited json', function() {
      tracker.visitItem('TPL1');
      tracker.visitItem('TPL4');
